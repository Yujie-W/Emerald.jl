...
function canopy_radiation_cache(config::EmeraldConfiguration{FT}, state::MultipleLayerSPACState{FT,DIM_CANOPY}, sza::FT, p_incl::SVector{DIM_INCL,FT}) where {FT,DIM_CANOPY,DIM_INCL}
    ...
    _leaf_optics = leaf_optical_properties.((config,), (state,), eachindex(state.chl), eachindex(config.WLSET.Λ)');
    ...
    return CanopyRadiationCache{FT,DIM_AZI,DIM_CANOPY,DIM_CANOPY+1,DIM_INCL,DIM_WL,DIM_AZI*DIM_INCL,DIM_CANOPY*DIM_WL}(
